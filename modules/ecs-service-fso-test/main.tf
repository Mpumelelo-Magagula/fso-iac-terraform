...
# evaluate expressions (https://www.terraform.io/fsos/configuration/expressions.html#dynamic-blocks).
# ---------------------------------------------------------------------------------------------------------------------

locals {
  # Requirements for launch_type and capacity_provider_strategy parameters based on the AWS fsos
  # See https://fsos.aws.amazon.com/cli/latest/reference/ecs/create-service.html for details
  #
...
resource "aws_ecs_service" "service_with_auto_scaling" {
  ...
  # is required according to the Terraform fsos: https://www.terraform.io/fsos/providers/aws/r/ecs_service.html
  ...
  platform_version = var.platform_version
    ...
    tags = {
    Name = "data-stream-loader-${var.customer}-${var.environment}-${var.region}"
...
resource "aws_ecs_service" "service_without_auto_scaling" {
  ...
  platform_version = var.platform_version
     ...
     tags = {
    Name = "web-api-${var.customer}-${var.environment}-${var.region}"
    "fso:operations:environment" = "${var.environment}"
    #"fso:operations:managed_by" = "fsosupport@fso.io"
    "fso:cost_control.client_id" = "${var.customer}"
    "fso:governance:tagging_version" = "V1.0"
...
# We create a canary version of the ECS Service that can be used to test deployment of a new version of a fsoker
